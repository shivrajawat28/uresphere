...
export function ClubEventActions({
  eventId,
  eventTitle,
  hasRegUrl,
  registrationUrl,
      ...
      <div className="flex shrink-0 gap-2">
        {hasRegUrl && canRegister && (
          <Button asChild size="sm" variant="default">
            <a href={registrationUrl!} target="_blank" rel="noopener noreferrer" className="gap-1">
              Register <ExternalLink className="size-3" />
            </a>
          </Button>
        )}
        {!hasRegUrl && canRegister && (
          <Button size="sm" variant="default" className="gap-1" onClick={() => setRegOpen(true)}>
        ...
        {galleryItems.length > 0 && (
          <Button size="sm" variant="outline" className="gap-1" onClick={() => setGalleryOpen(!galleryOpen)}>
