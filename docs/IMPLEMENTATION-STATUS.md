...
| Feature | Status | Notes |
|---|---|---|
| Signup (name, phone, email, college, password, confirm) | DONE | College resolved via directory; `college_id` passed to signup trigger |
| Signin with email OR phone | DONE | Phone resolved to registered email via private profile table; super admins are redirected to `/admin` server-side after login |
| Forgot / reset password | DONE | Supabase `resetPasswordForEmail` + recovery flow |
| Logout | DONE | |
...
| Session restore | DONE | `proxy.ts` session refresh |
...
| Academic hierarchy | DONE | Degree → Year → Branch → Subject → Unit drill-down, resources + calendar |
